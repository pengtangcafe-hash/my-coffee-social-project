...
<head>
  ...
  <style>
    ...
    .vf-kpi-row {{ display:flex; flex-wrap:wrap; gap:10px; margin-bottom:18px; }}
    .vf-kpi-card {{ background:var(--card); border:1px solid var(--card-border); border-radius:14px;
      padding:12px 16px; flex:1 1 130px; min-width:0; }}
    .vf-kpi-label {{ font-size:.72rem; color:var(--text-muted); font-weight:700; text-transform:uppercase;
      letter-spacing:.05em; margin-bottom:2px; }}
    .vf-kpi-val {{ font-size:1.22rem; font-weight:900; color:var(--text); font-variant-numeric:tabular-nums;
      overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }}
    .vf-kpi-fixed .vf-kpi-val {{ color:#e53935; }}
    .vf-kpi-var   .vf-kpi-val {{ color:#1e88e5; }}
    .vf-kpi-cat {{ font-size:.88rem!important; }}
    .vf-bills {{ display:flex; flex-direction:column; gap:12px; }}
    ...
    .vf-receipt-head {{ padding:10px 14px; display:flex; align-items:center; gap:8px; flex-wrap:wrap;
      min-height:38px; border-radius:14px 14px 0 0; }}
    ...
    .vf-receipt-body {{ padding:12px 16px 14px; font-feature-settings:"tnum"; }}
    .vf-receipt-label {{ font-weight:700; font-size:.92rem; color:var(--text); margin-bottom:6px;
      display:flex; align-items:center; flex-wrap:wrap; gap:4px; }}
    .vf-readonly-badge {{ font-size:.66rem; color:var(--text-muted); background:var(--nav-active);
      border-radius:6px; padding:2px 7px; font-weight:600; }}
    .vf-receipt-lines {{ font-size:.78rem; color:var(--text-muted); margin:4px 0 4px; }}
    .vf-receipt-line {{ padding:2px 0; border-bottom:1px dashed var(--card-border); }}
    .vf-receipt-line:last-child {{ border-bottom:none; }}
    .vf-receipt-dashes {{ font-size:.7rem; color:var(--card-border); letter-spacing:3px; margin:8px 0 6px;
      overflow:hidden; white-space:nowrap; }}
    .vf-receipt-amount {{ font-size:1.4rem; font-weight:900; color:var(--text); margin:4px 0 6px;
      font-variant-numeric:tabular-nums; }}
    .vf-receipt-note {{ font-size:.75rem; color:var(--text-muted); margin-bottom:8px; }}
...
function stkOpenPurchase() {{
  var html='<div class="dc-modal-head"><h3>📦 บันทึกซื้อเข้า</h3>'
    ...
    +'</div>'
    +'<div style="display:flex;gap:10px;flex-wrap:wrap">'
    +'<div class="dc-field" style="flex:1;min-width:130px"><label>📅 วันที่</label>'
...
function stkSavePurchase() {{
  ...
  var rec={{date:date,ing:ing,qty:qty,unit:unit,size:size,price:price,note:note}};
  var doSave=function() {{
...
function vfPurchaseBills() {{
  ...
  return Object.keys(byDate).map(function(date) {{
    ...
    var total=items.reduce(function(s,p) {{ return s+(p.qty&&p.price?p.qty*p.price:0); }},0);
    return {{id:'stock-'+date,date:date,group:'variable',category:'ingredient',label:'ซื้อเข้าสต็อก',amount:total,color:'#1e88e5',slip:'',note:'',items:items,readonly:true}};
  }});
}}
function vfReceiptCard(b) {{
  ...
  var headHtml='<div class="vf-receipt-head" style="background:'+escapeHtml(color)+'">'
    ...
    +'<span class="vf-receipt-date">'+vfFmtDate(b.date)+'</span>'
    +'</div>';
  var linesHtml='';
  if (b.items&&b.items.length) {{
    var lineRows=b.items.map(function(p) {{
      var lineAmt=p.qty&&p.price?p.qty*p.price:null;
      return '<div class="vf-receipt-line">'+escapeHtml(p.ing||'')
        +(p.qty?' · '+p.qty+(p.unit?' '+p.unit:''):'')
        +(lineAmt?' · '+vfFmtMoney(lineAmt):'')+'</div>';
    }}).join('');
    linesHtml='<div class="vf-receipt-lines">'+lineRows+'</div>';
  }}
  var readonlyBadge=b.readonly?'<span class="vf-readonly-badge">จากซื้อเข้าสต็อก</span>':'';
  ...
  return '<div class="vf-receipt-card">'
    ...
    +linesHtml
    +'<div class="vf-receipt-dashes">- - - - - - - - - - - - - - - - - - - -</div>'
    +'<div class="vf-receipt-amount">'+vfFmtMoney(b.amount||0)+'</div>'
    +noteHtml
    +slipBtn
    +'</div></div>';
}}
function vfMockBills() {{
  var t=vfTodayISO();
  return [
    {{id:'mock1',date:t,group:'fixed',category:'rent',label:'ค่าเช่าร้าน (ตัวอย่าง)',amount:8000,color:'#e53935',slip:'',note:'',readonly:true}},
    {{id:'mock2',date:t,group:'fixed',category:'utility',label:'ค่าน้ำ-ไฟ (ตัวอย่าง)',amount:1200,color:'#fb8c00',slip:'',note:'',readonly:true}},
    {{id:'mock3',date:t,group:'variable',category:'ingredient',label:'ซื้อเข้าสต็อก (ตัวอย่าง)',amount:3500,color:'#1e88e5',slip:'',note:'',readonly:true,
      items:[
    ...
    {{id:'mock4',date:t,group:'variable',category:'packaging',label:'บรรจุภัณฑ์ (ตัวอย่าง)',amount:850,color:'#3949ab',slip:'',note:'',readonly:true}}
  ];
}}
function renderVarfixView() {{
  ...
  var catTotals={{}};
  displayBills.forEach(function(b) {{ catTotals[b.category]=(catTotals[b.category]||0)+(b.amount||0); }});
  var topCat=Object.keys(catTotals).sort(function(a,b) {{ return catTotals[b]-catTotals[a]; }})[0];
  var topCatLabel=topCat?vfCatLabel(topCat):'—';
  ...
  var kpiHtml='<div class="vf-kpi-row">'
    +'<div class="vf-kpi-card"><div class="vf-kpi-label">รวมรายจ่าย</div><div class="vf-kpi-val">'+vfFmtMoney(totalAmt)+'</div></div>'
    +'<div class="vf-kpi-card vf-kpi-fixed"><div class="vf-kpi-label">Fixed</div><div class="vf-kpi-val">'+vfFmtMoney(fixedAmt)+'</div></div>'
    +'<div class="vf-kpi-card vf-kpi-var"><div class="vf-kpi-label">Variable</div><div class="vf-kpi-val">'+vfFmtMoney(varAmt)+'</div></div>'
    +'<div class="vf-kpi-card"><div class="vf-kpi-label">จ่ายมากสุด</div><div class="vf-kpi-val vf-kpi-cat">'+escapeHtml(topCatLabel)+'</div></div>'
    +'</div>';
  var toolbarHtml='<div class="dc-toolbar"><span class="spacer"></span><button class="dc-btn primary" onclick="vfOpenAddExpense()">+ บันทึกรายจ่าย</button></div>';
  ...
  root.innerHTML=tabsHtml+kpiHtml+toolbarHtml+mockHeaderHtml+'<div class="vf-bills">'+billsHtml+'</div>';
...
function vfSaveExpense() {{
  ...
  var doSave=function() {{
    var exp={{id:Date.now().toString(36)+Math.random().toString(36).slice(2,6),
      date:date,group:group,category:category,label:label,amount:amount,color:color,slip:'',note:note}};
