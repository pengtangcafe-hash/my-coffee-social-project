...
function dcOpenPwModal() {{
  var html = '<div class="dc-modal-head"><h3>🔒 ปลดล็อกการแก้ไข</h3><button class="dc-x" onclick="dcCloseModal()" aria-label="ปิด">✕</button></div>'
    + '<div class="dc-modal-scroll">'
    + '<div class="dc-field" style="margin-bottom:6px"><label>ใส่รหัสเพื่อแก้ไขเมนู / สูตร</label>'
...
function dcPwSubmit() {{
  ...
  dcSha256((i.value || '').trim()).then(function(h) {{
    if (h === DC_EDIT_HASH) {{ dcCloseModal(); dcEdit = true; renderDrinkCosts(); showToast('ปลดล็อกแล้ว ✓'); }}
