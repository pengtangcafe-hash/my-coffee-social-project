...
<head>
  ...
  <style>
    ...
    .dc-modal {{ background: var(--card); border: 1px solid var(--card-border); border-radius: 20px;
      width: 100%; max-width: 720px; padding: 24px; box-shadow: 0 40px 90px -30px rgba(0,0,0,.6);
    ...
    .dc-modal-head {{ display: flex; align-items: center; justify-content: space-between; gap: 12px; margin-bottom: 18px; }}
    ...
    .dc-modal-foot {{ display: flex; flex-wrap: wrap; gap: 10px; justify-content: flex-end; margin-top: 18px; }}
    ...
    .dc-cat-list {{ max-height: 48vh; overflow-y: auto; padding-right: 4px; margin-top: 8px; }}
...
function renderDrinkCosts() {{
  ...
  h.push('<div class="dc-controls"><div class="grp"><span class="lbl">ช่องทาง</span>' + chBtns + '</div><div class="grp"><span class="lbl">เรียงตาม</span>' + sortBtns + '</div></div>');
...
function dcSetModalBody(html) {{
  ...
  bd.addEventListener('mousedown', function(e) {{ if (e.target === bd) dcCloseModal(); }});
...
// ── Catalog editor ──
function dcOpenCatalog() {{ dcRenderCatalogModal(); }}
function dcRenderCatalogModal() {{
  var keys = Object.keys(DCS.catalog).sort(function(a, b) {{ return a.localeCompare(b, 'th'); }});
  var rows = keys.map(function(k) {{
    var c = DCS.catalog[k];
    return '<div class="dc-cat-row" data-orig="' + escapeHtml(k) + '">'
