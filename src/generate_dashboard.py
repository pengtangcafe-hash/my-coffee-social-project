...
function renderVarfixView() {{
  ...
  root.innerHTML=tabsHtml+summaryHtml+toolbarHtml+mockHeaderHtml+'<div class="vf-bills">'+billsHtml+'</div>';
...
function vfSaveExpense() {{
  ...
  var doSave=function() {{
    var exp={{id:Date.now().toString(36)+Math.random().toString(36).slice(2,6),
      date:date,group:group,category:category,label:label,amount:amount,color:color,slip:'',note:note,pay:pay,vendor:vendor}};
