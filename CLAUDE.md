...
## มาตรฐานการทดสอบ (QA) — ต้องทำก่อนบอกว่า "เสร็จ" หรือ commit
ผู้ใช้เป็นเจ้าของร้าน ไม่ใช่สายเทคนิค — ภาระทำให้ "ใช้งานได้จริง" เป็นของ assistant ทั้งหมด
- **เทสแบบผู้ใช้จริง ไม่ใช่แค่เช็คค่าในโค้ด** — จำลอง click/scroll/พิมพ์ ด้วย event จริง (`dispatchEvent`)
  อย่าเชื่อแค่ `el.scrollTop=N` หรือเรียกฟังก์ชันตรงๆ เพราะมันผ่านได้ทั้งที่ UI จริงพัง
- **ทดสอบบนจอเล็กเสมอ** — ใช้ `preview_resize` จำลองจอจริง (เช่น ~1366×600 / โน้ตบุ๊ก) ทุกครั้งที่ทำ modal/overlay/dropdown
- **คิด edge case ก่อนเป็นค่าเริ่มต้น** — เนื้อหายาวเกินจอ, dropdown ชนขอบล่าง (ต้องเด้งขึ้น/จำกัดสูง), คลิกพลาดต้องไม่ทำงานหาย, event flow (rebuild กลาง event ทำ element หลุด DOM)
- **เดินครบ flow** — เปิด → กรอก → เลือก → เลื่อน → บันทึก ให้ครบจริงก่อนปิดงาน
- **เช็ค console errors** ทุกครั้ง (`preview_console_logs --level error`)
- **modal ที่กรอกข้อมูล**: หัว/ปุ่มตรึง (flex-shrink:0) + เนื้อหา `min-height:0; overflow:auto`; อย่าให้คลิกพื้นหลังปิดทิ้ง (เสียงาน) — ปิดทาง ✕/ยกเลิก/Esc
- **อธิบายให้ผู้ใช้เทสง่าย** — บอกเป็นสเต็ป ("กดตรงนี้ ควรเห็นแบบนี้") + เตือน **Ctrl+Shift+R** เลี่ยงแคชทุกครั้งที่ deploy
