...
## 🗄️ BACKBAR — ระบบหลังร้านอื่นๆ (ทำใหม่)

ทุกหน้า BACKBAR **ซิงก์กับ Google Sheet เดียวกัน** (เชื่อม URL ครั้งเดียวจากหน้าต้นทุนเครื่องดื่ม ใช้ได้ทุกหน้า) · เก็บ local-first + ดันขึ้น Sheet อัตโนมัติ

### 🔒 ระบบล็อก/เบลอ (กันคนอื่นดู/แก้)
- หน้า BACKBAR + Quest จะ **เบลอเมื่อล็อกอยู่** (เห็นลางๆ อ่านไม่ออก) — ปลดล็อกด้วยรหัส `ohcloudrouter`
- ปลดล็อก 2 ทาง: การ์ด "🔓 ปลดล็อกเพื่อดู" กลางจอ · chip มุมซ้ายล่าง (กดสลับล็อก↔ปลด)
- ปิดแท็บ/เบราว์เซอร์ = ล็อกกลับเอง

### 🔄 ซิงก์อัตโนมัติ + กันข้อมูลหาย
- **เปิดแอปทุกครั้ง = ซิงก์สองทางอัตโนมัติ** (มีของค้าง→ดันขึ้นก่อน แล้วดึงล่าสุด)
- เกราะกันพลาด: ถ้า Sheet ว่างแต่ในเครื่องมีข้อมูล → ไม่ดึงมาทับ + เตือน
- ⚠️ **แก้ Code.gs แล้วต้อง Deploy → Manage deployments → ✏️ → New version** (URL เดิม ไม่เปลี่ยน) — อย่ากด "New deployment" (URL จะเปลี่ยน)

### 📦 สต็อกหลังบ้าน
- บันทึกซื้อเข้า / ยอดขายต่อเมนู → คำนวณ **คงเหลือ** อัตโนมัติ + แจ้งเตือนกระพริบเมื่อต่ำกว่าเกณฑ์
- ปุ่ม **✏️ แก้ไขรายตัว** บนการ์ด (จำนวน/หน่วย/รูป) · ปุ่ม **📎 อัปรูป** วัตถุดิบขึ้น Drive

### 💰 ต้นทุนผันแปรและคงที่
- บันทึกรายจ่าย (Fixed/Variable) · **บิลจัดกลุ่มตามเดือน** หุบ/กางได้ · แนบสลิป (อัปขึ้น Drive)
- การ์ด **ประวัติ & เปรียบเทียบย้อนหลัง** (เทียบเดือน + กราฟเทรนด์) · กราฟแท่งแนวนอนเรียงยอด
- กรอง: วันนี้ / 7 วัน / เดือนนี้ / **เดือนที่แล้ว** / ปีนี้

### 🏷️ อัพเดทราคาวัตถุดิบ (Makro price watch)
- เทียบ **ราคาตลาด Makro** vs **ราคาที่ซื้อจริง** (จากบันทึกซื้อเข้า) + กราฟแนวโน้มขึ้น/ลง
- จัดกลุ่มตามร้าน: **Makro / ปราณี / 7even** (หุบ/กางได้ + โลโก้ร้าน) · กล่องราคาเขียว + ระบุขนาดแพ็คชัด
- **วิธีอัปราคา:** บอก Claude **"อัพเดทราคาวัตถุดิบ"** → Claude ดึงราคาล่าสุดจาก makro.pro (ตาม URL ใน `data/ingredient-prices.json`) → บันทึกสถิติ (เฉพาะเมื่อราคาเปลี่ยน) → push
- กด **"+ บันทึกราคาเอง"** เพิ่มราคาที่ซื้อเองได้ (แก้ได้ ซิงก์เก็บ)

### ⚔️ 90 Days Quest (ใต้ ภาพรวม)
- to-do แบบ gamified: เควส · ความสำเร็จ · แผน(วัน/สัปดาห์/เดือน) · Loop · ประวัติ + นับวัน/streak/คำคม

### 📥 บันทึกข้อมูลนำเข้า POS (Loyverse)
- ดึงยอดขายจริงจาก Loyverse POS (ต้องตั้งค่าตาม `POS-SETUP-CHECKLIST.md` ก่อน)

### 🖼️ อัปรูปขึ้น Google Drive (สลิป/วัตถุดิบ/โลโก้)
ทุกปุ่มอัปรูปใช้ Apps Script เดียวกัน — เก็บใน Drive แยกโฟลเดอร์:
`ร้านกาแฟ-สลิป` (แยกปี/เดือน) · `ร้านกาแฟ-รูปวัตถุดิบ` · `ร้านกาแฟ-โลโก้ร้าน`
> ครั้งแรกต้อง **รันฟังก์ชัน authorizeDrive ใน Apps Script** เพื่ออนุญาต Drive (ดู `POS-SETUP-CHECKLIST.md`)
...
| **ต้นทุนเครื่องดื่ม** (Backbar) | ต้นทุน/กำไร/สูตร 4 ช่องทาง (+Grab) + แก้ไขได้ + ซิงก์ Google Sheet |
| **สต็อกหลังบ้าน** (Backbar) | คงเหลือ + แจ้งเตือน + แก้รายตัว + อัปรูปวัตถุดิบ |
| **ต้นทุนผันแปรและคงที่** (Backbar) | รายจ่าย Fixed/Variable + กลุ่มเดือน + กราฟ + สลิป |
| **อัพเดทราคาวัตถุดิบ** (Backbar) | ราคา Makro vs ซื้อจริง จัดกลุ่มตามร้าน + กราฟแนวโน้ม |
| **90 Days Quest** (ใต้ภาพรวม) | to-do gamified: เควส/ความสำเร็จ/แผน/Loop/ประวัติ |
| ตรวจสอบยอดขายจริง & ทุนรวม / นำเข้า POS | ดึงยอดขายจริงจาก Loyverse (ตั้งค่าตาม POS-SETUP-CHECKLIST.md) |
| บันทึกอัปเดต | ประวัติการอัปเดตทั้งหมด จัดกลุ่มตามเดือน (Platforms + Intelligence) |
